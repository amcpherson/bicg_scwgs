...
ggplot(as_tibble(gaps$Tab) %>% mutate(k=factor(1:30))) +
...
gap_stat <- clusGap(mat.pca$x[, 1:50], FUN = pam, K.max = 30, B = 50)
fviz_gap_stat(gap_stat)


pure.pbmc <- neighborPurity(mat.pca$x[, 1:50], clusters=clust)
...
Alternative clustering approaches are possible, see [this excellent scRNA tutorial](https://bioconductor.org/books/release/OSCA/clustering.html) for ideas.  A graph based clustering approach such as that illustrated below could work better for clustering copy number data as graph based methods will not favour spherical clusters.
...
We can also create merged cluster level copy number data and Create merged cluster level copy number:
